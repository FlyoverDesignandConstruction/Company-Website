...
(function () {
  ...
  const TESTIMONIALS = [
    { name: "U Kyaw Min",      role: "Property Developer, Yangon",  quote: "Flyover delivered our commercial tower ahead of schedule with exceptional craftsmanship. A true partner from blueprint to handover." },
    { name: "Daw Hnin Ei",     role: "Homeowner, Naypyidaw",        quote: "They turned our dream villa into reality. Every detail considered, every promise kept. The team felt like family by the end." },
    { name: "Mr. James Lin",   role: "Hotel Operator, Mandalay",    quote: "The heritage renovation was masterful — preserving history while elevating function. We could not recommend them more highly." },
    { name: "U Aung Thu",      role: "Restaurant Owner",            quote: "Stunning interior design that completely transformed our space. Bookings doubled within the first month of reopening." },
    { name: "Daw Su Su Khin",  role: "Apartment Resident",          quote: "From the first consultation to final walkthrough, professionalism and care defined every interaction with Flyover." },
    { name: "U Zaw Win",       role: "Industrial Client",           quote: "Their M&E and high-tension expertise is unmatched in Myanmar. On time, on budget, and built to last." },
    { name: "Ma Thiri Lwin",   role: "Boutique Owner, Yangon",      quote: "Flyover understood our brand language instantly. The finished space is a daily compliment to our customers." },
    { name: "Mr. Hideo Tanaka",role: "Foreign Investor",            quote: "Transparent, fluent, and rigorous. Working with Flyover gave us complete confidence in our Myanmar investment." }
  ];

  function tmInitials(name) {
    const parts = name.replace(/^(Mr\.|Mrs\.|Ms\.|Daw|U|Ma)\s*/i, "").trim().split(/\s+/);
  ...
  function tmMakeCard(t) {
    const card = document.createElement("figure");
    card.className = "tm-card";
    card.setAttribute("aria-label", `Testimonial from ${t.name}`);
    card.innerHTML = `
      <span class="tm-open-quote" aria-hidden="true">\u201C</span>
      <blockquote class="tm-quote">${t.quote}</blockquote>
      <div class="tm-divider"></div>
      <div class="tm-author-row">
        <div class="tm-avatar" aria-hidden="true">${tmInitials(t.name)}</div>
        <div class="tm-author-info">
          <div class="tm-author-name">${t.name}</div>
          <div class="tm-author-role">${t.role}</div>
        </div>
        <div class="tm-stars" aria-label="5 stars">${Array(5).fill(tmStarHTML()).join("")}</div>
      </div>`;
    return card;
  }

  function tmFillTrack(el, items) {
    if (!el) return;
    const frag = document.createDocumentFragment();
    [...items, ...items].forEach(t => frag.appendChild(tmMakeCard(t)));
    el.appendChild(frag);
  }
